...
public class CustomEntities {
    ...
    public static void attribute() {
        for (EntityType<?> type : types) {
            FabricDefaultAttributeRegistry.register(type, (DefaultAttributeContainer)EntityNPCInterface.createMobAttributes().build());
        }
    }
    ...
    private static <T extends Entity> EntityType<T> registerNpc(Class<? extends Entity> c, String name, EntityType.EntityFactory<T> factoryIn) {
        EntityType.Builder builder = EntityType.Builder.create(factoryIn, (SpawnGroup)SpawnGroup.field_6294);
        ...
        types.add(type);
        Registry.register((Registry)Registries.ENTITY_TYPE, (Identifier)registryName, (Object)type);
        if (CustomNpcs.FixUpdateFromPre_1_12) {
            registryName = new Identifier("customnpcs." + name);
            Registry.register((Registry)Registries.ENTITY_TYPE, (Identifier)registryName, (Object)builder.build(registryName.toString()));
        }
        return type;
    }
    ...
    private static <T extends Entity> EntityType<T> registerNewentity(Class<? extends Entity> c, String name, EntityType.EntityFactory<T> factoryIn, int range, int update, boolean velocity, float width, float height) {
        EntityType.Builder builder = EntityType.Builder.create(factoryIn, (SpawnGroup)SpawnGroup.field_17715);
