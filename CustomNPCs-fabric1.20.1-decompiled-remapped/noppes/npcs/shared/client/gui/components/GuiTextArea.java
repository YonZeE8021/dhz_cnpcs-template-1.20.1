...
Element {
    ...
    public GuiTextArea(int id, int x, int y, int width, int height, String text) {
        super(x, y, width, height, (Text)Text.literal((String)text));
        this.id = id;
        this.x = x;
        this.y = y;
        this.width = width;
    ...
    @Override
    public void render(DrawContext graphics, int xMouse, int yMouse) {
        if (!this.visible) {
            return;
        }
        ...
        if (this.clicked) {
            this.clicked = ((MouseHelperMixin)MinecraftClient.getInstance().mouse).getActiveButton() == 0;
        ...
        if (this.clickScrolling) {
            this.clickScrolling = ((MouseHelperMixin)MinecraftClient.getInstance().mouse).getActiveButton() == 0;
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (wordHightLight != null) {
                ...
                while (m.find()) {
                    ...
                    graphics.fill(this.x + 1 + s, posY2, this.x + 1 + e2, posY2 + this.container.lineHeight + 1, -1728033792);
                }
            }
            ...
            font.draw(graphics.getMatrices(), data.getFormattedString(), this.x + 1, yPos, -2039584);
        ...
        if (this.hasVerticalScrollbar()) {
            ...
            RenderSystem.setShaderTexture((int)0, (Identifier)GuiCustomScrollNop.resource);
    ...
    public boolean charTyped(char c, int i) {
        ...
        if (SharedConstants.isValidChar((char)c)) {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (keyCode == 262) {
            Matcher m;
            int j = 1;
            if (Screen.hasControlDown() && ((m = this.container.regexWord.matcher(this.text.substring(this.cursorPosition))).find() && m.start() > 0 || m.find())) {
                j = m.start();
            }
            this.setCursor(this.cursorPosition + j, Screen.hasShiftDown());
            return true;
        }
        if (keyCode == InputUtil.fromTranslationKey((String)"key.keyboard.up").getCode()) {
            this.setCursor(this.cursorUp(), Screen.hasShiftDown());
            return true;
        }
        if (keyCode == InputUtil.fromTranslationKey((String)"key.keyboard.down").getCode()) {
