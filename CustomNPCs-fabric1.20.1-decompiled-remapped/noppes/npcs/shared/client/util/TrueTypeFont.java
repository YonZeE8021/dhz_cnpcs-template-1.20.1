...
public class TrueTypeFont {
    ...
    public void fillGradient(Matrix4f m, float x, float y, float textureX, float textureY, float width, float height, int r, int g, int b) {
        ...
        tessellator.begin(VertexFormat.DrawMode.field_27382, VertexFormats.POSITION_TEXTURE_COLOR);
        tessellator.vertex(m, x, y + height, (float)zLevel).texture(textureX * f, (textureY + height) * f1).color(r, g, b, 255).next();
        tessellator.vertex(m, x + width, y + height, (float)zLevel).texture((textureX + width) * f, (textureY + height) * f1).color(r, g, b, 255).next();
        tessellator.vertex(m, x + width, y, (float)zLevel).texture((textureX + width) * f, textureY * f1).color(r, g, b, 255).next();
        tessellator.vertex(m, x, y, (float)zLevel).texture(textureX * f, textureY * f1).color(r, g, b, 255).next();
