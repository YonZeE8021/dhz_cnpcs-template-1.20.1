...
implements IProjectile {
    ...
    public void setHeading(double x, double y, double z) {
        float varF = ((EntityProjectile)this.entity).hasGravity() ? (float)Math.sqrt((x -= ((EntityProjectile)this.entity).getX()) * x + (z -= ((EntityProjectile)this.entity).getZ()) * z) : 0.0f;
        float angle = ((EntityProjectile)this.entity).getAngleForXYZ(x, y -= ((EntityProjectile)this.entity).getY(), z, varF, false);
        float acc = 20.0f - (float)MathHelper.floor((float)((float)((EntityProjectile)this.entity).accuracy / 5.0f));
        ((EntityProjectile)this.entity).setVelocity(x, y, z, angle, acc);
    }

    @Override
    public void setHeading(float yaw, float pitch) {
        ((EntityProjectile)this.entity).prevYaw = yaw;
        ((EntityProjectile)this.entity).prevPitch = pitch;
        ((EntityProjectile)this.entity).setYaw(yaw);
        ((EntityProjectile)this.entity).setPitch(pitch);
        ...
        float acc = 20.0f - (float)MathHelper.floor((float)((float)((EntityProjectile)this.entity).accuracy / 5.0f));
        ((EntityProjectile)this.entity).setVelocity(varX, varY, varZ, -pitch, acc);
    }
