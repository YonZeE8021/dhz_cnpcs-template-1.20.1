...
implements ICustomGui {
    ...
    @Override
    public void update() {
        if (((ServerPlayerEntity)this.player.getMCEntity()).currentScreenHandler instanceof ContainerCustomGui) {
            Packets.send(this.player.getMCEntity(), new PacketGuiData(this.getRootGui().toNBT()));
        }
        ((ContainerCustomGui)((ServerPlayerEntity)this.player.getMCEntity()).currentScreenHandler).setGui(this.getActiveGui(), (PlayerEntity)this.player.getMCEntity());
    }

    @Override
    public void update(ICustomGuiComponent component) {
        if (((ServerPlayerEntity)this.player.getMCEntity()).currentScreenHandler instanceof ContainerCustomGui) {
            Packets.send(this.player.getMCEntity(), new PacketGuiComponentUpdate(component.getUniqueID(), ((CustomGuiComponentWrapper)component).toNBT(new NbtCompound())));
        }
    }

    public ICustomGui fromNBT(NbtCompound tag) {
    ...
    public NbtCompound toNBT() {
        ...
        tag.put("backgroundRect", (NbtElement)this.background.toNBT(new NbtCompound()));
        tag.put("components", (NbtElement)this.getComponentNbt());
        tag.put("scrolling_components", (NbtElement)this.scrollingPanel.getComponentNbt());
        if (this.parent == null) {
            tag.putInt("slotSize", this.getActiveGui().getSlots().size());
        }
        if (this.subgui != null) {
            tag.put("subgui", (NbtElement)this.subgui.toNBT());
        }
        return tag;
    }
