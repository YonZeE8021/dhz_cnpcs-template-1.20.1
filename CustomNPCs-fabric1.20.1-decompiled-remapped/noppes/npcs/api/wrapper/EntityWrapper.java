...
implements IEntity {
    ...
    public void damage(float damage, IEntity source) {
        if (source.getMCEntity() instanceof PlayerEntity) {
            ...
            RegistryEntry.Reference damageTypeHolder = this.entity.getWorld().getRegistryManager().get(RegistryKeys.field_42534).entryOf(NpcDamageSource.NPC);
    ...
    public boolean inFire() {
        return this.entity.getWorld().getStatesInBox(this.entity.getBoundingBox()).anyMatch(state -> state.isIn(BlockTags.field_21952));
    }
    ...
    public IRayTrace rayTraceBlock(double distance, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox) {
        ...
        BlockHitResult result = this.entity.getWorld().raycast(new RaycastContext(vec3d, vec3d2, RaycastContext.ShapeType.field_17559, stopOnLiquid ? RaycastContext.FluidHandling.field_1347 : RaycastContext.FluidHandling.field_1348, this.entity));
        if (result.getType() == HitResult.Type.field_1333) {
            return null;
        }
    ...
    public IEntity[] rayTraceEntities(double distance, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox) {
        ...
        Vec3d vec3d1 = this.entity.getRotationVec(1.0f);
        Vec3d vec3d2 = vec3d.add(vec3d1.x * distance, vec3d1.y * distance, vec3d1.z * distance);
        BlockHitResult result = this.entity.getWorld().raycast(new RaycastContext(vec3d, vec3d2, RaycastContext.ShapeType.field_17558, stopOnLiquid ? RaycastContext.FluidHandling.field_1347 : RaycastContext.FluidHandling.field_1348, this.entity));
        if (result.getType() != HitResult.Type.field_1333) {
    ...
    private IEntity[] findEntityOnPath(double distance, Vec3d vec3d, Vec3d vec3d1) {
        List list = this.entity.getWorld().getOtherEntities(this.entity, this.entity.getBoundingBox().expand(distance));
