...
implements IQuestHandler {
    ...
    public void saveCategory(QuestCategory category) {
        ...
        this.categories.put(category.id, category);
        Packets.sendAll(new PacketSyncUpdate(category.id, 3, category.writeNBT(new NbtCompound())));
    }
    ...
    public void saveQuest(QuestCategory category, Quest quest) {
        ...
        try {
            NBTJsonUtil.SaveFile(file, quest.writeToNBTPartial(new NbtCompound()));
            if (file2.exists()) {
                file2.delete();
            }
            file.renameTo(file2);
            Packets.sendAll(new PacketSyncUpdate(category.id, 2, quest.save(new NbtCompound())));
        }
        catch (Exception e) {
            e.printStackTrace();
        }
