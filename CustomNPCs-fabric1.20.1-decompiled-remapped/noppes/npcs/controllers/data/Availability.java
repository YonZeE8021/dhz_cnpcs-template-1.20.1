...
IAvailability {
    ...
    private void initScore(String objective) {
        ...
        for (ServerWorld level : CustomNpcs.Server.getWorlds()) {
            ...
            if (so == null) continue;
            board.addScoreboardObjective(so);
        }
    }
    ...
    public boolean isAvailable(PlayerEntity player) {
        long time;
        if (this.daytime == EnumDayTime.Day && (time = player.getWorld().getTimeOfDay() % 24000L) > 12000L) {
            return false;
        }
        if (this.daytime == EnumDayTime.Night && (time = player.getWorld().getTimeOfDay() % 24000L) < 12000L) {
            return false;
        }
    ...
    private boolean scoreboardAvailable(PlayerEntity player, String objective, EnumAvailabilityScoreboard type, int value) {
        ...
        ScoreboardObjective sbObjective = player.getScoreboard().getNullableObjective(objective);
        if (sbObjective == null) {
            return false;
        }
        if (!player.getScoreboard().playerHasObjective(player.getName().getString(), sbObjective)) {
            return false;
        }
        int i = player.getScoreboard().getPlayerScore(player.getName().getString(), sbObjective).getScore();
    ...
    private boolean factionAvailable(int id, EnumAvailabilityFaction stance, EnumAvailabilityFactionType available, PlayerEntity player) {
        ...
        PlayerFactionData data = PlayerData.get((PlayerEntity)player).factionData;
    ...
    public boolean dialogAvailable(int id, EnumAvailabilityDialog en, PlayerEntity player) {
        ...
        boolean hasRead = PlayerData.get((PlayerEntity)player).dialogData.dialogsRead.contains(id);
    ...
    public boolean isAvailable(IPlayer player) {
        return this.isAvailable((PlayerEntity)player.getMCEntity());
    }
