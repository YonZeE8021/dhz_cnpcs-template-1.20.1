...
implements INPCRanged {
    ...
    public int getDelayRNG() {
        ...
        if (this.maxDelay - this.minDelay > 0) {
            delay += this.npc.getWorld().random.nextInt(this.maxDelay - this.minDelay);
        }
        return delay;
    }
    ...
    public SoundEvent getSoundEvent(int type) {
        ...
        Identifier res = new Identifier(sound);
        SoundEvent ev = (SoundEvent)Registries.SOUND_EVENT.get(res);
        if (ev != null) {
            return ev;
        }
        return SoundEvent.of((Identifier)res);
    }
