...
implements INPCDisplay {
    ...
    private BossBar.Color bossColor = BossBar.Color.field_5788;
    ...
    public NbtCompound save(NbtCompound nbttagcompound) {
        ...
        nbttagcompound.putBoolean("OverlayGlowing", this.overlayGlowing);
        nbttagcompound.putByte("UsingSkinUrl", this.skinType);
        if (this.playerProfile != null) {
            NbtCompound nbttagcompound1 = new NbtCompound();
            NbtHelper.writeGameProfile((NbtCompound)nbttagcompound1, (GameProfile)this.playerProfile);
            nbttagcompound.put("SkinUsername", (NbtElement)nbttagcompound1);
        }
        ...
        nbttagcompound.put("VisibleAvailability", (NbtElement)this.availability.save(new NbtCompound()));
    ...
    public void readToNBT(NbtCompound nbttagcompound) {
        ...
        if (nbttagcompound.contains("OverlayGlowing")) {
            this.overlayGlowing = nbttagcompound.getBoolean("OverlayGlowing");
        }
        this.playerProfile = null;
        if (this.skinType == 1) {
            if (nbttagcompound.contains("SkinUsername", 10)) {
                this.playerProfile = NbtHelper.toGameProfile((NbtCompound)nbttagcompound.getCompound("SkinUsername"));
            } else if (nbttagcompound.contains("SkinUsername", 8) && !StringHelper.isEmpty((String)nbttagcompound.getString("SkinUsername"))) {
        ...
        this.npc.calculateDimensions();
    }

    public void loadProfile() {
        if (this.playerProfile != null && !StringHelper.isEmpty((String)this.playerProfile.getName())) {
            if (this.npc.getServer() == null) {
                SkullBlockEntity.loadProperties((GameProfile)this.playerProfile, profile -> {
                    this.playerProfile = profile;
                });
            } else {
                this.playerProfile = DataDisplay.getGameprofile(this.npc.getServer(), this.playerProfile);
            }
        }
    }
    ...
    private static GameProfile getGameprofile(MinecraftServer server, GameProfile profile) {
        try {
            if (profile == null || StringHelper.isEmpty((String)profile.getName()) || profile.isComplete() && profile.getProperties().containsKey((Object)"textures")) {
                return profile;
            }
            GameProfile gameprofile = server.getUserCache().findByName(profile.getName()).orElse(null);
            if (gameprofile == null) {
                return profile;
            }
    ...
    public void setBossColor(int color) {
        if (color < 0 || color >= BossBar.Color.values().length) {
            throw new CustomNPCsException("Invalid Boss Color: " + color, new Object[0]);
        }
        this.bossColor = BossBar.Color.values()[color];
