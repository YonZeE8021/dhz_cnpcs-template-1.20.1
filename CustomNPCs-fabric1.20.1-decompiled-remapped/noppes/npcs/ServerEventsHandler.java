...
ServerLivingEntityEvents.AfterDeath {
    ...
    public ActionResult interact(PlayerEntity player, World world, Hand hand, Entity entity, @Nullable EntityHitResult hitResult) {
        ...
        if (item.isEmpty() || hand != Hand.field_5808) {
    ...
    private void doQuest(PlayerEntity player, LivingEntity entity, boolean all) {
        ...
        String entityName = Registries.ENTITY_TYPE.getId((Object)entity.getType()).toString();
        if (entity instanceof PlayerEntity) {
            entityName = "Player";
        }
        for (QuestData data : playerdata.activeQuests.values()) {
            ...
            if (data.quest.type == 4 && all) {
                List list = player.getWorld().getNonSpectatingEntities(PlayerEntity.class, entity.getBoundingBox().expand(10.0, 10.0, 10.0));
    ...
    public static boolean allowCommandMessage(ParseResults<ServerCommandSource> parseRes) {
        ...
        if (command.startsWith("give ")) {
            try {
                ...
                Collection players = EntityArgumentType.getPlayers((CommandContext)context, (String)"targets");
        ...
        if (command.startsWith("time ")) {
            try {
                CustomNpcs.Server.submit(() -> {
                    List players = CustomNpcs.Server.getPlayerManager().getPlayerList();
