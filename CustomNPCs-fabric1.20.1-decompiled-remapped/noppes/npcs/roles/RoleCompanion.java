...
extends RoleInterface {
    ...
    private void doEating() {
        ...
        if (this.npc.getWorld().isClient) {
            ...
            for (int j = 0; j < 2; ++j) {
                ...
                this.npc.getWorld().addParticle((ParticleEffect)new ItemStackParticleEffect(ParticleTypes.field_11218, eating), vec31.x, vec31.y, vec31.z, vec3.x, vec3.y + 0.05, vec3.z);
            }
        } else {
            --this.eatingTicks;
            if (this.eatingTicks <= 0) {
                ...
                if (this.inventory.removeItem(eating, 1)) {
                    ...
                    this.npc.playSound(SoundEvents.field_19149, 0.5f, this.npc.getRandom().nextFloat() * 0.1f + 0.9f);
                }
                this.eatingDelay = 20;
                this.npc.setRoleData("");
                this.eating = null;
                ...
                Random rand = this.npc.getRandom();
                this.npc.playSound(SoundEvents.field_20614, 0.5f + 0.5f * (float)rand.nextInt(2), (rand.nextFloat() - rand.nextFloat()) * 0.2f + 1.0f);
            }
        }
    }
    ...
    public NbtCompound save(NbtCompound compound) {
        ...
        for (EnumCompanionTalent talent : this.talents.keySet()) {
            ...
            c.putInt("Exp", this.talents.get((Object)talent).intValue());
            list.add((Object)c);
        }
    ...
    private boolean isWeapon(ItemStack item) {
        ...
        return item.getItem() instanceof SwordItem || item.getItem() instanceof BowItem || item.getItem() == Item.fromBlock((Block)Blocks.field_10445);
    }
    ...
    public boolean canWearWeapon(IItemStack stack) {
        ...
        if (item == Item.fromBlock((Block)Blocks.field_10445)) {
    ...
    private double getSwordDamage(IItemStack item) {
        ...
        Multimap map = item.getMCItemStack().getAttributeModifiers(EquipmentSlot.field_6173);
    ...
    public void setStats() {
        ...
        if (ranged > 0 && weapon != null) {
            ...
            if (ranged > 0 && item == Item.fromBlock((Block)Blocks.field_10445)) {
                this.npc.inventory.setProjectile(weapon);
            }
            if (ranged > 0 && item instanceof BowItem) {
                this.npc.inventory.setProjectile(NpcAPI.Instance().getIItemStack(new ItemStack((ItemConvertible)Items.field_8107)));
            }
        }
    ...
    public float getDamageAfterArmorAbsorb(DamageSource source, float damage) {
        ...
        if (!source.isIn(DamageTypeTags.field_43116)) {
    ...
    private void damageArmor(float damage) {
        ...
        while (ita.hasNext()) {
            ...
            hasArmor = true;
            item.getMCItemStack().damage((int)damage, (LivingEntity)this.npc, entity -> entity.sendEquipmentBreakStatus(EquipmentSlot.fromTypeIndex((EquipmentSlot.Type)EquipmentSlot.Type.field_6178, (int)((Integer)entry.getKey()))));
    ...
    public void attackedEntity(Entity entity) {
        ...
        weapon.getMCItemStack().damage(1, (LivingEntity)this.npc, e -> e.sendEquipmentBreakStatus(EquipmentSlot.field_6173));
