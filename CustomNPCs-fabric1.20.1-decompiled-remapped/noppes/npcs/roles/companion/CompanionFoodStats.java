...
public class CompanionFoodStats {
    ...
    public void onUpdate(EntityNPCInterface npc) {
        ...
        if (this.foodExhaustionLevel > 4.0f) {
            ...
            } else if (enumdifficulty != Difficulty.field_5801) {
                this.foodLevel = Math.max(this.foodLevel - 1, 0);
            }
        }
        if (npc.getWorld().getGameRules().getBoolean(GameRules.field_19395) && this.foodLevel >= 18 && npc.getHealth() > 0.0f && npc.getHealth() < npc.getMaxHealth()) {
            ++this.foodTimer;
            ...
            ++this.foodTimer;
            if (this.foodTimer >= 80) {
                if (npc.getHealth() > 10.0f || enumdifficulty == Difficulty.field_5807 || npc.getHealth() > 1.0f && enumdifficulty == Difficulty.field_5802) {
