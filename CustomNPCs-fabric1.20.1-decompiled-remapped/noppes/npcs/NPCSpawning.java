...
public class NPCSpawning {
    public static void findChunksForSpawning(ServerWorld level) {
        ...
        while (iterator.hasNext() && (levelchunk = (chunkHolder = (ChunkHolder)iterator.next()).getWorldChunk()) != null) {
            ...
            Biome biome = (Biome)level.getBiome(pos.getStartPos()).comp_349();
            if (!SpawnController.instance.hasSpawnList(level.getRegistryManager().get(RegistryKeys.BIOME).getId((Object)biome))) continue;
            ...
            sectionManager.forEachIntersects((TypeFilter)EntityType.field_6097, bb.expand(4.0), e -> {
                entities.add(e);
    ...
    private static void spawnChunk(ServerWorld level, WorldChunk chunk) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            Identifier name = level.getRegistryManager().get(RegistryKeys.BIOME).getId((Object)((Biome)level.getBiome(pos).comp_349()));
    ...
    public static int countNPCs(ServerWorld level) {
        int count = 0;
        Iterable list = level.iterateEntities();
    ...
    public static void performLevelGenSpawning(ServerWorldAccess level, Biome biome, int x, int z, Random rand) {
        if (biome.getSpawnSettings().getCreatureSpawnProbability() >= 1.0f || biome.getSpawnSettings().getCreatureSpawnProbability() < 0.0f || !SpawnController.instance.hasSpawnList(level.getRegistryManager().get(RegistryKeys.BIOME).getId((Object)biome))) {
            return;
        }
        int tries = 0;
        block0: while (rand.nextFloat() < biome.getSpawnSettings().getCreatureSpawnProbability() && ++tries <= 20) {
            SpawnData data = SpawnController.instance.getRandomSpawnData(level.getRegistryManager().get(RegistryKeys.BIOME).getId((Object)biome));
            int size = 16;
    ...
    private static boolean spawnData(SpawnData data, ServerWorldAccess level, BlockPos pos) {
        ...
        if (!entityliving.canSpawn((WorldAccess)level, SpawnReason.field_16459) || !entityliving.canSpawn((WorldView)level)) {
            return false;
        }
    ...
    public static float getLightLevel(WorldView level, BlockPos pos) {
        int blockLight = level.getLightLevel(LightType.field_9282, pos);
        int skyLight = level.getLightLevel(LightType.field_9284, pos);
    ...
    public static boolean canCreatureTypeSpawnAtLocation(SpawnData data, WorldView level, BlockPos pos) {
        ...
        boolean flag = block1 != Blocks.field_9987 && block1 != Blocks.field_10499;
    ...
    private static BlockPos getTopNonCollidingPos(WorldView p_208498_0_, EntityType<?> p_208498_1_, int p_208498_2_, int p_208498_3_) {
        BlockPos blockpos;
        int i = p_208498_0_.getTopY(Heightmap.Type.field_13203, p_208498_2_, p_208498_3_);
        BlockPos.Mutable blockpos$mutable = new BlockPos.Mutable(p_208498_2_, i, p_208498_3_);
        if (p_208498_0_.getDimension().comp_643()) {
            do {
                blockpos$mutable.move(Direction.field_11033);
        ...
        if (p_208498_0_.getBlockState(blockpos = blockpos$mutable.down()).canPathfindThrough((BlockView)p_208498_0_, blockpos, NavigationType.field_50)) {
            return blockpos;
        }
