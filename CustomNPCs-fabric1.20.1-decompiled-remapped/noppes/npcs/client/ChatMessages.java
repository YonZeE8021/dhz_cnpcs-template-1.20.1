...
implements IChatMessages {
    ...
    protected static final RenderLayer type = RenderLayer.of((String)"chatbubble", (VertexFormat)VertexFormats.POSITION_COLOR_LIGHT, (VertexFormat.DrawMode)VertexFormat.DrawMode.field_27382, (int)256, (boolean)false, (boolean)false, (RenderLayer.MultiPhaseParameters)RenderLayer.MultiPhaseParameters.builder().cull(new RenderPhase.Cull(true)).lightmap(new RenderPhase.Lightmap(true)).program(sharder).build(true));
    protected static final RenderLayer typeDepth = RenderLayer.of((String)"chatbubbledepth", (VertexFormat)VertexFormats.POSITION_COLOR_LIGHT, (VertexFormat.DrawMode)VertexFormat.DrawMode.field_27382, (int)256, (boolean)false, (boolean)true, (RenderLayer.MultiPhaseParameters)RenderLayer.MultiPhaseParameters.builder().cull(new RenderPhase.Cull(true)).transparency(TRANSLUCENT_TRANSPARENCY).program(sharder).lightmap(new RenderPhase.Lightmap(true)).depthTest(new RenderPhase.DepthTest("always", 519)).build(false));
    ...
    public void render(MatrixStack poseStack, VertexConsumerProvider typeBuffer, VertexConsumer ivertex, float textScale, boolean depth, int lightmapUV) {
        ...
        for (TextBlockClient block : this.messages.values()) {
            for (Text chat : block.lines) {
                ...
                Objects.requireNonNull(font);
                font.draw(chat, f, (float)(index * 9), black, false, matrix, typeBuffer, !depth ? TextRenderer.TextLayerType.field_33994 : TextRenderer.TextLayerType.field_33993, 0, lightmapUV);
                ++index;
            }
        }
        poseStack.pop();
