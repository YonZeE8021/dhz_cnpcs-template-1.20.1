...
ITextChangeListener {
    ...
    @Override
    public void init() {
        this.imageWidth = (int)((double)this.width * 0.88);
        this.imageHeight = (int)((double)this.imageWidth * 0.56);
        if ((double)this.imageHeight > (double)this.height * 0.95) {
            this.imageHeight = (int)((double)this.height * 0.95);
        ...
        if (this.activeTab > 0) {
            ...
            this.getButton((int)103).active = this.languages.size() > 0;
    ...
    private String getConsoleText() {
        ...
        for (Map.Entry<Long, String> entry : map.entrySet()) {
            builder.insert(0, String.valueOf(new Date(entry.getKey())) + entry.getValue() + "\n");
        }
    ...
    public void buttonEvent(GuiButtonNop guibutton) {
        ...
        if (guibutton.id == 109) {
            this.setScreen((Screen)new ConfirmLinkScreen(bo -> {
                if (bo) {
                    Util.getOperatingSystem().open("http://www.kodevelopment.nl/minecraft/customnpcs/scripting");
                }
                this.setScreen(this);
        ...
        if (guibutton.id == 110) {
            this.setScreen((Screen)new ConfirmLinkScreen(bo -> {
                if (bo) {
                    Util.getOperatingSystem().open("http://www.kodevelopment.nl/customnpcs/api/");
                }
                this.setScreen(this);
        ...
        if (guibutton.id == 111) {
            this.setScreen((Screen)new ConfirmLinkScreen(bo -> {
                if (bo) {
                    Util.getOperatingSystem().open("https://github.com/Noppes/CustomNPCsAPI");
                }
                this.setScreen(this);
        ...
        if (guibutton.id == 112) {
            this.setScreen((Screen)new ConfirmLinkScreen(bo -> {
                if (bo) {
                    Util.getOperatingSystem().open("https://github.com/Noppes/cnpcs-scripting-examples");
                }
                this.setScreen(this);
        ...
        if (guibutton.id == 103) {
            this.handler.setLanguage(guibutton.getMessage().getString());
        }
        if (guibutton.id == 104) {
    ...
    public void setGuiData(NbtCompound compound) {
        NbtList data = compound.getList("Languages", 10);
        HashMap<String, List<String>> languages = new HashMap<String, List<String>>();
        for (int i = 0; i < data.size(); ++i) {
            NbtCompound comp = data.getCompound(i);
            ArrayList<String> scripts = new ArrayList<String>();
            NbtList list = comp.getList("Scripts", 8);
