...
implements ScreenEvents.AfterInit {
        ...
        if (!TileBuilder.Compiled) {
            ...
            try {
                for (int i = 0; i < schem.size && i < 25000; ++i) {
                    ...
                    if (state.getRenderType() == BlockRenderType.field_11455 || state.getRenderType() != BlockRenderType.field_11458) continue;
    ...
    public void afterInit(MinecraftClient client, Screen screen, int scaledWidth, int scaledHeight) {
        if (screen instanceof InventoryScreen && CustomNpcs.InventoryGuiEnabled) {
            screen.children().add(new InventoryTabVanilla().init(screen));
            screen.children().add(new InventoryTabFactions().init(screen));
            screen.children().add(new InventoryTabQuests().init(screen));
            screen.drawables.add(new InventoryTabVanilla().init(screen));
            screen.drawables.add(new InventoryTabFactions().init(screen));
            screen.drawables.add(new InventoryTabQuests().init(screen));
        }
    }
}
