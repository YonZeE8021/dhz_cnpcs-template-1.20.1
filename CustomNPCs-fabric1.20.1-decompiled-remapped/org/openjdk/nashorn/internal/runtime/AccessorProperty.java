...
extends Property {
    ...
    private MethodHandle debug(MethodHandle mh, Class<?> forType, Class<?> type, String tag) {
        ...
        assert (context != null);
        return context.addLoggingToHandle(ObjectClassGenerator.class, Level.INFO, mh, 0, true, () -> tag + " '" + this.getKey() + "' (property=" + Debug.id(this) + ", slot=" + this.getSlot() + " " + this.getClass().getSimpleName() + " forType=" + MethodHandleFactory.stripName(forType) + ", type=" + MethodHandleFactory.stripName(type) + ")");
    }
