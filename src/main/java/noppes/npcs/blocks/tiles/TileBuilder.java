...
extends BlockEntity {
    ...
    public static void tick(World level, BlockPos pos, BlockState state, TileBuilder tile) {
        ...
        List<EntityNPCInterface> list = level.getNonSpectatingEntities(EntityNPCInterface.class, new Box(pos, pos).expand(32.0, 32.0, 32.0));
    ...
    public Stack<BlockData> getBlock() {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!state.isFullCube((BlockView)EmptyBlockView.INSTANCE, BlockPos.ORIGIN) && !bo && state.getBlock() != Blocks.AIR) {
