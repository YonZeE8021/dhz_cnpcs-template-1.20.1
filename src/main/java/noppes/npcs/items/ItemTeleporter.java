...
extends Item {
    ...
    public static boolean onEntitySwing(ItemStack stack, LivingEntity livingEntity) {
        ...
        BlockHitResult movingobjectposition = livingEntity.getWorld().raycast(new RaycastContext(vector3d, vector3d1, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.ANY, (Entity)livingEntity));
        ...
        if (movingobjectposition.getType() == HitResult.Type.BLOCK) {
            ...
            while (livingEntity.getWorld().getBlockState(pos).getBlock() != Blocks.AIR) {
                pos = pos.up();
            }
