...
implements ICustomNpc {
    ...
    public void sayTo(IPlayer player, String message) {
        ((EntityNPCInterface)this.entity).say((PlayerEntity)player.getMCEntity(), new Line(message));
    }
    ...
    public void giveItem(IPlayer player, IItemStack item) {
        ((EntityNPCInterface)this.entity).givePlayerItem((PlayerEntity)player.getMCEntity(), item.getMCItemStack());
    }

    @Override
    public String executeCommand(String command) {
        if (!((EntityNPCInterface)this.entity).getServer().areCommandBlocksEnabled()) {
            throw new CustomNPCsException("Command blocks need to be enabled to executeCommands", new Object[0]);
        }
        return NoppesUtilServer.runCommand(this.entity, ((EntityNPCInterface)this.entity).getName().getString(), command, null);
    }
    ...
    public void setRotation(float rotation) {
        ...
        if (((EntityNPCInterface)this.entity).ais.orientation != r) {
            ...
            Packets.sendNearby(this.entity, new PacketNpcRotationUpdate(((EntityNPCInterface)this.entity).getId(), ((EntityNPCInterface)this.entity).ais.orientation));
        }
    }
