...
implements IBlock {
    ...
    private void setPropertyValue(BlockState state, Property<?> p, Comparable<?> c) {
        this.level.getMCLevel().setBlockState(this.pos, state.with((Property)p, (Comparable)c), 3);
    }
    ...
    public void interact(int side) {
        ...
        this.getMCBlockState().onUse((World)w, (PlayerEntity)EntityNPCInterface.CommandPlayer, Hand.MAIN_HAND, new BlockHitResult(Vec3d.ZERO, Direction.byId((int)side), this.pos, true));
    }
}
