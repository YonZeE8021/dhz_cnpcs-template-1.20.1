...
public class CobblemonHelper {
    ...
    public static boolean isPokemon(Entity entity) {
        ...
        Identifier typeResLoc = Registries.ENTITY_TYPE.getId(entity.getType());
        return typeResLoc.equals(new Identifier("cobblemon", "pokemon"));
    }
    ...
    public static EntityModel getPokemonModel(Entity entity) {
        ...
        try {
            ...
            model.getClass().getMethod("setProfileScale", Float.TYPE).invoke(model, Float.valueOf(1.0f));
            model.getClass().getMethod("setPortraitScale", Float.TYPE).invoke(model, Float.valueOf(1.0f));
        }
        catch (Exception exception) {
            // empty catch block
        }
