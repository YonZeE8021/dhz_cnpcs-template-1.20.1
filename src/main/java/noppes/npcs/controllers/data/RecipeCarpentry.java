...
implements IRecipe {
    ...
    public RecipeCarpentry(Identifier location, int width, int height, DefaultedList<Ingredient> recipe, ItemStack result) {
        super(location, "customnpcs", CraftingRecipeCategory.MISC, width, height, recipe, result);
        this.name = location.getPath();
    }

    public RecipeCarpentry(Identifier location, String name) {
        super(location, "customnpcs", CraftingRecipeCategory.MISC, 4, 4, DefaultedList.of(), ItemStack.EMPTY);
        this.name = name;
    }
    ...
    public static RecipeCarpentry createRecipe(Identifier location, RecipeCarpentry recipe, ItemStack par1ItemStack, Object ... limbSwingAmountArrayOfObj) {
        ...
        for (var9 = 0; var9 < var5 * var6; ++var9) {
            ...
            if (var14.containsKey(Character.valueOf(var18))) {
                ingredients.add(var9, Ingredient.ofStacks((ItemStack[])new ItemStack[]{((ItemStack)var14.get(Character.valueOf(var18))).copy()}));
                continue;
            }
    ...
    public DefaultedList<ItemStack> getRemainingItems(RecipeInputInventory inventoryCrafting) {
        DefaultedList list = DefaultedList.ofSize((int)inventoryCrafting.size(), ItemStack.EMPTY);
        for (int i = 0; i < list.size(); ++i) {
            ItemStack itemstack = inventoryCrafting.getStack(i);
            if (itemstack.getItem().hasRecipeRemainder()) {
                list.set(i, new ItemStack((ItemConvertible)itemstack.getItem().getRecipeRemainder()));
                continue;
            }
