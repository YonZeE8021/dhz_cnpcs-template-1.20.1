...
extends PacketBasic {
    ...
    protected void handle() {
        ...
        if (this.name.equals("heal")) {
            for (int k = 0; k < 6; ++k) {
                world.addParticle((ParticleEffect)ParticleTypes.INSTANT_EFFECT, this.posX + (rand.nextDouble() - 0.5) * (double)this.width, this.posY + rand.nextDouble() * (double)this.height, this.posZ + (rand.nextDouble() - 0.5) * (double)this.width, 0.0, 0.0, 0.0);
                world.addParticle((ParticleEffect)ParticleTypes.EFFECT, this.posX + (rand.nextDouble() - 0.5) * (double)this.width, this.posY + rand.nextDouble() * (double)this.height, this.posZ + (rand.nextDouble() - 0.5) * (double)this.width, 0.0, 0.0, 0.0);
            }
        }
    }
}
