...
IJobFarmer {
    ...
    private void pluck() {
        ...
        if (this.npc.nearPosition(pos) || this.walkTicks++ > 400) {
            ...
            if (b instanceof CropBlock && ((CropBlock)b).isMature(state)) {
                ...
                LootContextParameterSet.Builder builder = new LootContextParameterSet.Builder((ServerWorld)this.npc.getWorld()).add(LootContextParameters.ORIGIN, Vec3d.ofCenter((Vec3i)pos)).add(LootContextParameters.TOOL, this.npc.getMainHandStack()).add(LootContextParameters.BLOCK_STATE, state).addOptional(LootContextParameters.BLOCK_ENTITY, this.npc.getWorld().getBlockEntity(pos));
                LootTable loottable = this.npc.getServer().getLootManager().getLootTable(b.getLootTableId());
                ObjectArrayList<ItemStack> l = loottable.generateLoot(builder.build(LootContextTypes.BLOCK));
                ...
                if (l.isEmpty()) {
                    ...
                    this.holding = fl.get(this.npc.getRandom().nextInt(fl.size()));
                }
    ...
    public EnumSet<Goal.Control> getFlags() {
        return EnumSet.of(Goal.Control.MOVE);
    }
