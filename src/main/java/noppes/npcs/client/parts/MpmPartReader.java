...
public class MpmPartReader {
    ...
    private static MpmPart loadPart(Identifier location) {
        ...
        try (InputStream stream = r.getInputStream();
            ...
            LogWriter.error("Error in " + location, e);
    ...
    public static <T extends Enum<T>> List<T> jsonEnumList(Class<T> type, JsonElement el) {
    ...
    public static void Notify(Text message) {
        MutableText chatcomponenttranslation = ((MutableText)message).formatted(new Formatting[]{Formatting.GRAY, Formatting.ITALIC});
