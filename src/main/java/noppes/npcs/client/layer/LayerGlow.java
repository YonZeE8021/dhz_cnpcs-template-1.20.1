...
extends FeatureRenderer<T, M> {
    ...
    public void render(MatrixStack matrixStackIn, VertexConsumerProvider typeBuffer, int packedLightIn, T npc, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (((EntityNPCInterface)(npc)).textureGlowLocation == null) {
            ((EntityNPCInterface)(npc)).textureGlowLocation = new Identifier(((EntityNPCInterface)(npc)).display.getOverlayTexture());
        }
        VertexConsumer ivertexbuilder = null;
        ivertexbuilder = ((EntityNPCInterface)(npc)).display.isOverlayGlowing() ? typeBuffer.getBuffer(RenderLayer.getEntityTranslucentEmissive((Identifier)((EntityNPCInterface)(npc)).textureGlowLocation)) : typeBuffer.getBuffer(RenderLayer.getEntityTranslucent((Identifier)((EntityNPCInterface)(npc)).textureGlowLocation));
