...
implements ISliderListener {
    ...
    protected NbtCompound original = new NbtCompound();
    ...
    public void init() {
        ...
        if (this.entity == null) {
            ...
            if (!gui.getData(this.entity).isEmpty()) {
                ...
                this.client.setScreen((Screen)new GuiCreationEntities(this.npc));
                return;
            }
        }
        if (this.entity == null) {
    ...
    @Override
    public void render(DrawContext graphics, int x, int y, float f) {
    ...
    @Override
    public void save() {
        NbtCompound newCompound = this.playerdata.save();
        Packets.sendServer(new SPacketMenuSave(EnumMenuType.DISPLAY, this.npc.display.save(new NbtCompound())));
