...
extends GuiBasic {
    ...
    public void init() {
        ...
        Registry<Biome> biomeRegistry = this.player.getWorld().getRegistryManager().get(RegistryKeys.BIOME);
        for (Biome base : biomeRegistry) {
            if (base == null || biomeRegistry.getId(base) == null || this.data.biomes.contains(biomeRegistry.getId(base).toString())) continue;
            biomes.add(biomeRegistry.getId(base).toString());
        }
    ...
    public void buttonEvent(GuiButtonNop guibutton) {
        ...
        if (button.id == 3) {
            ...
            for (Biome base : biomeRegistry) {
                ...
                this.data.biomes.add(biomeRegistry.getId(base));
            }
            this.scroll1.clearSelection();
            this.scroll2.clearSelection();
            this.init();
