...
implements IGuiData {
    ...
    public static void addTopMenu(RoleCompanion role, Screen screen, int active) {
        ...
        if (screen instanceof GuiNPCInterface) {
            ...
            button = new GuiMenuTopIconButton(gui, 1, ((GuiNPCInterface)gui).guiLeft + 4, ((GuiNPCInterface)gui).guiTop - 27, "menu.stats", new ItemStack((ItemConvertible)Items.BOOK));
            ((GuiBasic)gui).addTopButton(button);
            button = new GuiMenuTopIconButton(gui, 2, button, "companion.talent", new ItemStack((ItemConvertible)Items.NETHER_STAR));
            ((GuiBasic)gui).addTopButton(button);
            if (role.hasInv()) {
                button = new GuiMenuTopIconButton(gui, 3, button, "inv.inventory", new ItemStack((ItemConvertible)Blocks.CHEST));
                ((GuiBasic)gui).addTopButton(button);
            }
            if (role.companionJobInterface.getType() != EnumCompanionJobs.NONE) {
                ((GuiBasic)gui).addTopButton(new GuiMenuTopIconButton(gui, 4, button, "job.name", new ItemStack((ItemConvertible)Items.CARROT)));
            }
        ...
        if (screen instanceof GuiContainerNPCInterface) {
            ...
            button = new GuiMenuTopIconButton(gui, 1, ((GuiContainerNPCInterface)gui).guiLeft + 4, ((GuiContainerNPCInterface)gui).guiTop - 27, "menu.stats", new ItemStack((ItemConvertible)Items.BOOK));
            ...
            if (role.companionJobInterface.getType() != EnumCompanionJobs.NONE) {
                ((GuiBasicContainer)gui).addTopButton(new GuiMenuTopIconButton(gui, 4, button, "job.name", new ItemStack((ItemConvertible)Items.CARROT)));
            }
