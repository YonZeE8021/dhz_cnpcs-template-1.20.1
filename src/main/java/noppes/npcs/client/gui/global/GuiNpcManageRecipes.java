...
ITextfieldListener {
    ...
    private Map<String, Identifier> data = new HashMap<String, Identifier>();
    private ContainerManageRecipes container;
    private String selected = null;
    private Identifier slot;

    public GuiNpcManageRecipes(ContainerManageRecipes container, PlayerInventory inv, Text titleIn) {
    ...
    public void buttonEvent(GuiButtonNop guibutton) {
        ...
        if (button.id == 3) {
            ...
            RecipeCarpentry recipe = new RecipeCarpentry(new Identifier("customnpcs", (String)name), (String)name);
    ...
    public void setGuiData(NbtCompound compound) {
        ...
        this.container.setRecipe(recipe, this.player.getWorld().getRegistryManager());
    ...
    @Override
    protected void drawBackground(DrawContext graphics, float f, int x, int y) {
        super.drawBackground(graphics, f, x, y);
        ...
        for (int i = 0; i < this.container.width; ++i) {
            for (int j = 0; j < this.container.width; ++j) {
                graphics.drawTexture(this.slot, this.guiLeft + i * 18 + 7, this.guiTop + j * 18 + 34, 0, 0, 18, 18);
            }
        }
        graphics.drawTexture(this.slot, this.guiLeft + 86, this.guiTop + 60, 0, 0, 18, 18);
    }

    @Override
    public void setData(Vector<String> list, Map<String, Integer> data) {
        String name = this.scroll.getSelected();
        this.data = new HashMap<String, Identifier>();
        for (String s : data.keySet()) {
            String[] arr = s.split("\\|");
            this.data.put(arr[0], new Identifier(arr[1]));
        }
    ...
    public void unFocused(GuiTextFieldNop guiNpcTextField) {
        ...
        if (!name.isEmpty() && !this.data.containsKey(name)) {
            ...
            this.data.put(this.container.recipe.name, new Identifier("customnpcs", this.container.recipe.name));
            this.selected = name;
