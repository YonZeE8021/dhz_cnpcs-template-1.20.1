...
ISliderListener {
    ...
    private void showSpawn() {
        ...
        this.addSlider(new GuiSliderNop(this, 4, this.guiLeft + 4, this.guiTop + 47, 180, 20, (float)this.spawn.getWeight().getValue() / 100.0f));
    ...
    private String getTitle(int slot) {
        if (this.spawn.data.containsKey(slot)) {
            return this.spawn.data.get(Integer.valueOf((int)slot)).name;
        }
    ...
    public void buttonEvent(GuiButtonNop guibutton) {
        ...
        if (id == 1) {
            this.save();
            String name = I18n.translate((String)"gui.new", (Object[])new Object[0]);
            while (this.data.containsKey(name)) {
                name = name + "_";
            }
            SpawnData spawn = new SpawnData();
            spawn.name = name;
            Packets.sendServer(new SPacketNaturalSpawnSave(spawn.writeNBT(new NbtCompound())));
        }
    ...
    public void save() {
        ...
        if (this.spawn.id >= 0) {
            Packets.sendServer(new SPacketNaturalSpawnSave(this.spawn.writeNBT(new NbtCompound())));
        }
    }
    ...
    public void mouseDragged(GuiSliderNop guiNpcSlider) {
        guiNpcSlider.setMessage((Text)Text.translatable((String)"spawning.weightedChance").append(": " + (int)(guiNpcSlider.sliderValue * 100.0f)));
    }
