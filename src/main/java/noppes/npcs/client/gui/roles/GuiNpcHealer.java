...
extends GuiNPCInterface2 {
    ...
    public GuiNpcHealer(EntityNPCInterface npc) {
        ...
        Registry<StatusEffect> r = Registries.STATUS_EFFECT;
        for (StatusEffect effect : r) {
            Identifier rl = r.getId(effect);
            if (rl == null) continue;
            this.potions.put(rl.toString(), r.getRawId(effect));
        }
    }
    ...
    public void buttonEvent(GuiButtonNop guibutton) {
        ...
        if (button.id == 13) {
            ...
            Registry<StatusEffect> r = Registries.STATUS_EFFECT;
            for (StatusEffect effect : r) {
                this.job.effects.put(r.getRawId(effect), this.potency);
            }
            this.scroll1.clearSelection();
            this.scroll2.clearSelection();
            this.init();
