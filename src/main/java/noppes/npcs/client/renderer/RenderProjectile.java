...
extends EntityRenderer<T> {
    ...
    public void render(T projectile, float entityYaw, float partialTicks, MatrixStack matrixStack, VertexConsumerProvider buffer, int packedLight) {
        ...
        matrixStack.push();
        float scale = (float)((EntityProjectile)(projectile)).getSize() / 10.0f;
        ItemStack item = ((EntityProjectile)(projectile)).getItemDisplay();
        matrixStack.scale(scale, scale, scale);
        if (((EntityProjectile)(projectile)).isArrow()) {
            matrixStack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(MathHelper.lerp((float)partialTicks, (float)((EntityProjectile)(projectile)).prevYaw, (float)projectile.getYaw()) - 90.0f));
            matrixStack.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(MathHelper.lerp((float)partialTicks, (float)((EntityProjectile)(projectile)).prevPitch, (float)projectile.getPitch())));
            float f9 = (float)((EntityProjectile)(projectile)).arrowShake - partialTicks;
            if (f9 > 0.0f) {
        ...
        } else if (((EntityProjectile)(projectile)).is3D()) {
            matrixStack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(MathHelper.lerp((float)partialTicks, (float)((EntityProjectile)(projectile)).prevYaw, (float)projectile.getYaw()) - 180.0f));
            matrixStack.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(MathHelper.lerp((float)partialTicks, (float)((EntityProjectile)(projectile)).prevPitch, (float)projectile.getPitch())));
            matrixStack.translate(0.0f, -0.125f, 0.25f);
            if (item.getItem() instanceof BlockItem && Block.getBlockFromItem((Item)item.getItem()).getDefaultState().getRenderType() == BlockRenderType.ENTITYBLOCK_ANIMATED) {
            ...
            if (!this.crash) {
                try {
                    mc.getItemRenderer().renderItem(item, ModelTransformationMode.THIRD_PERSON_RIGHT_HAND, packedLight, OverlayTexture.DEFAULT_UV, matrixStack, buffer, null, 0);
                }
                catch (Throwable e) {
                    this.crash = true;
                }
                ...
                try {
                    mc.getItemRenderer().renderItem(item, ModelTransformationMode.NONE, packedLight, OverlayTexture.DEFAULT_UV, matrixStack, buffer, null, 0);
                }
                catch (Throwable ee) {
                    this.crash2 = true;
                }
            } else {
                mc.getItemRenderer().renderItem(new ItemStack((ItemConvertible)Blocks.DIRT), ModelTransformationMode.GROUND, packedLight, OverlayTexture.DEFAULT_UV, matrixStack, buffer, null, 0);
            }
        } else {
            matrixStack.scale(0.5f, 0.5f, 0.5f);
            matrixStack.multiply(this.dispatcher.camera.getRotation());
            matrixStack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(180.0f));
            mc.getItemRenderer().renderItem(item, ModelTransformationMode.GROUND, packedLight, OverlayTexture.DEFAULT_UV, matrixStack, buffer, null, 0);
        }
        if (!((EntityProjectile)(projectile)).is3D() || ((EntityProjectile)(projectile)).glows()) {
            // empty if block
        }
        matrixStack.pop();
    }
    ...
    public Identifier getTextureLocation(T par1Entity) {
        return ((EntityProjectile)(par1Entity)).isArrow() ? field_110780_a : SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE;
    }
