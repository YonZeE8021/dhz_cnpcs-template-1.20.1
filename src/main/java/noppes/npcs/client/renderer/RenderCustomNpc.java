...
extends RenderNPCInterface<T, M> {
    ...
    private FeatureRenderer renderLayer = new FeatureRenderer(null){
        ...
        public void render(MatrixStack mStack, VertexConsumerProvider typeBuffer, int lightmapUV, Entity p_225628_4_, float limbSwing, float limbSwingAmount, float partialTicks, float age, float netHeadYaw, float headPitch) {
            for (Object o : ((LivingRenderer2Mixin)(Object)RenderCustomNpc.this.renderEntity).layers()) {
                ((FeatureRenderer)o).render(mStack, typeBuffer, lightmapUV, (LivingEntity)RenderCustomNpc.this.entity, limbSwing, limbSwingAmount, partialTicks, age, netHeadYaw, headPitch);
            }
        }
    };
    ...
    public RenderCustomNpc(EntityRendererFactory.Context manager, M model) {
        ...
        this.renderModel = new BipedEntityModel(manager.getPart(EntityModelLayers.PLAYER)){
            ...
            public void animateModel(Entity npc, float animationPos, float animationSpeed, float partialTicks) {
                ...
                if (RenderCustomNpc.this.otherModel instanceof EntityModel) {
                    ...
                    em.handSwingProgress = RenderCustomNpc.this.getHandSwingProgress((T)RenderCustomNpc.this.entity, partialTicks);
    ...
    public Vec3d getRenderOffset(T npc, float partialTicks) {
        float xOffset = 0.0f;
        float yOffset = ((EntityCustomNpc)(npc)).currentAnimation == 0 ? ((EntityCustomNpc)(npc)).ais.bodyOffsetY / 10.0f - 0.5f : 0.0f;
        float zOffset = 0.0f;
        if (((EntityNPCInterface)(npc)).isAlive()) {
            if (((EntityNPCInterface)(npc)).isSleeping()) {
                xOffset = (float)(-Math.cos(Math.toRadians(180 - ((EntityCustomNpc)(npc)).ais.orientation)));
                zOffset = (float)(-Math.sin(Math.toRadians(((EntityCustomNpc)(npc)).ais.orientation)));
                yOffset += 0.14f;
            } else if (((EntityCustomNpc)(npc)).currentAnimation == 1 || npc.hasVehicle()) {
                yOffset -= 0.5f - ((EntityCustomNpc)(npc)).modelData.getLegsY() * 0.8f;
            } else if (((EntityNPCInterface)(npc)).isInSneakingPose()) {
                yOffset = (float)((double)yOffset - 0.125);
            }
        }
        return new Vec3d((double)xOffset, (double)(yOffset * ((float)((EntityCustomNpc)(npc)).display.getSize() / 5.0f)), (double)zOffset);
    }

    void hideParts() {
        if (this.npc instanceof EntityCustomNpc) {
            ModelData data = ModelData.get((EntityCustomNpc)this.npc);
            ((BipedEntityModel)this.npcmodel).leftLeg.visible = !data.hiddenParts.contains(BodyPart.LEFT_LEG);
            ((BipedEntityModel)this.npcmodel).rightLeg.visible = !data.hiddenParts.contains(BodyPart.RIGHT_LEG);
            ((BipedEntityModel)this.npcmodel).leftArm.visible = !data.hiddenParts.contains(BodyPart.LEFT_ARM);
            ((BipedEntityModel)this.npcmodel).rightArm.visible = !data.hiddenParts.contains(BodyPart.RIGHT_ARM);
            ((BipedEntityModel)this.npcmodel).body.visible = !data.hiddenParts.contains(BodyPart.BODY);
            ((BipedEntityModel)this.npcmodel).head.visible = !data.hiddenParts.contains(BodyPart.HEAD);
            boolean bl = ((BipedEntityModel)this.npcmodel).hat.visible = !data.hiddenParts.contains(BodyPart.HEAD);
    ...
    public void render(T npc, float entityYaw, float partialTicks, MatrixStack matrixStack, VertexConsumerProvider buffer, int packedLight) {
        ...
        this.entity = ((EntityCustomNpc)(npc)).modelData.getEntity((EntityNPCInterface)(npc));
        ...
        if (this.entity != null) {
            ...
            if (((EntityCustomNpc)(npc)).modelData.simpleRender) {
                this.renderEntity = null;
                matrixStack.push();
            ...
            if (render instanceof LivingEntityRenderer) {
                ...
                this.model = (M)(Object)this.renderModel;
                this.features.clear();
                this.features.add(this.renderLayer);
                this.features.add(new LayerGlow(this));
                if (render instanceof RenderCustomNpc) {
                    for (Object o : ((LivingRenderer2Mixin)(Object)this.renderEntity).layers()) {
                        FeatureRenderer<?, ?> layer = (FeatureRenderer<?, ?>)o;
            ...
            this.hideParts();
            List<FeatureRenderer<T, M>> list = this.features;
            for (FeatureRenderer<T, M> layer : list) {
                if (!(layer instanceof LayerPreRender)) continue;
                ((LayerPreRender)layer).preRender((EntityCustomNpc)(npc));
            }
        }
        ((BipedEntityModel)this.npcmodel).rightArmPose = this.getPose(npc, ((EntityNPCInterface)(npc)).getMainHandStack());
        ((BipedEntityModel)this.npcmodel).leftArmPose = this.getPose(npc, ((EntityNPCInterface)(npc)).getOffHandStack());
    ...
    protected void scale(T npc, MatrixStack matrixScale, float f) {
        if (this.renderEntity != null) {
            ...
            int size = ((EntityCustomNpc)(npc)).display.getSize();
            if (this.entity instanceof EntityNPCInterface) {
                ((EntityNPCInterface)this.entity).display.setSize(5);
            }
            EntityRenderer render = this.dispatcher.getRenderer((Entity)this.entity);
            if (!((EntityCustomNpc)(npc)).modelData.simpleRender && render instanceof LivingEntityRenderer) {
                ((LivingRenderer3Mixin)render).callScale(this.entity, matrixScale, this.partialTicks);
            }
            ((EntityCustomNpc)(npc)).display.setSize(size);
            matrixScale.scale(0.2f * (float)((EntityCustomNpc)(npc)).display.getSize(), 0.2f * (float)((EntityCustomNpc)(npc)).display.getSize(), 0.2f * (float)((EntityCustomNpc)(npc)).display.getSize());
        } else {
