...
implements ISchematic {
    ...
    private static BlockState setValue(BlockState state, Property<?> prop, String val) {
        Optional<?> optional = prop.parse(val);
        if (optional.isPresent()) {
            return state.with((Property)prop, (Comparable)optional.get());
        }
        return state;
    }
    ...
    public NbtCompound getNBT() {
        ...
        for (Map.Entry<String, BlockState> entry : this.blockIds.entrySet()) {
            comp.putString("" + Block.getRawIdFromState((BlockState)entry.getValue()), Registries.BLOCK.getId(entry.getValue().getBlock()).toString());
        }
