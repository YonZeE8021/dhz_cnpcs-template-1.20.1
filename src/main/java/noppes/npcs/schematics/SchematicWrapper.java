...
public class SchematicWrapper {
    ...
    public void place(int x, int y, int z, int flag) {
        ...
        if (state == null || flag == 1 && !state.isFullCube((BlockView)EmptyBlockView.INSTANCE, BlockPos.ORIGIN) && state.getBlock() != Blocks.AIR || flag == 2 && (state.isFullCube((BlockView)EmptyBlockView.INSTANCE, BlockPos.ORIGIN) || state.getBlock() == Blocks.AIR)) {
            return;
        }
    ...
    public BlockState rotationState(BlockState state, int rotation) {
        ...
        for (Property prop : state.getProperties()) {
            Direction direction;
            if (!(prop instanceof DirectionProperty) || (direction = (Direction)state.get(prop)) == Direction.UP || direction == Direction.DOWN) continue;
    ...
    public NbtCompound getNBTSmall() {
        ...
        for (int i = 0; i < this.size && i < 25000; ++i) {
            ...
            list.add(NbtHelper.fromBlockState((BlockState)this.schema.getBlockState(i)));
        }
