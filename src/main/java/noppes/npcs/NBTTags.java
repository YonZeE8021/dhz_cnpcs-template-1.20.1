...
public class NBTTags {
    public static void getItemStackList(NbtList tagList, DefaultedList<ItemStack> items) {
        ...
        for (int i = 0; i < tagList.size(); ++i) {
            ...
            try {
                items.set(nbttagcompound.getByte("Slot") & 0xFF, ItemStack.fromNbt((NbtCompound)nbttagcompound));
                continue;
            }
            catch (ClassCastException e) {
                items.set(nbttagcompound.getInt("Slot"), ItemStack.fromNbt((NbtCompound)nbttagcompound));
            }
        }
    }
    ...
    public static DefaultedList<Ingredient> getIngredientList(NbtList tagList) {
        ...
        for (int i = 0; i < tagList.size(); ++i) {
            NbtCompound nbttagcompound = tagList.getCompound(i);
            list.add(nbttagcompound.getByte("Slot") & 0xFF, Ingredient.ofStacks((ItemStack[])new ItemStack[]{ItemStack.fromNbt((NbtCompound)nbttagcompound)}));
        }
        return list;
    }
    ...
    public static NbtCompound NBTMerge(NbtCompound data, NbtCompound merge) {
        ...
        Set<String> names = merge.getKeys();
