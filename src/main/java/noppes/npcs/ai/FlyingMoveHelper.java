...
extends MoveControl {
    ...
    public void tick() {
        if (this.state == MoveControl.State.MOVE_TO && this.courseChangeCooldown-- <= 0) {
            ...
            if (length > 0.5 && this.isNotColliding(vector3d, MathHelper.ceil((double)length))) {
                double speed = this.entity.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).getValue() / 2.5;
