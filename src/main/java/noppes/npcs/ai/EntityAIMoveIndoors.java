...
extends Goal {
    ...
    public EntityAIMoveIndoors(PathAwareEntity par1Mob) {
    ...
    public boolean canStart() {
        if (this.theCreature.getWorld().isDay() && !this.theCreature.getWorld().isRaining() || this.theCreature.getWorld().getDimension().hasSkyLight()) {
            return false;
        }
        BlockPos pos = new BlockPos((int)this.theCreature.getX(), (int)this.theCreature.getBoundingBox().minY, (int)this.theCreature.getZ());
        if (!this.level.isSkyVisible(pos) && this.level.getLuminance(pos) > 8) {
            return false;
        }
