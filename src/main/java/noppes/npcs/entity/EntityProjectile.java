...
extends ThrownEntity {
    ...
    protected void initDataTracker() {
        this.dataTracker.startTracking(ItemStackThrown, ItemStack.EMPTY);
        this.dataTracker.startTracking(Velocity, 10);
        this.dataTracker.startTracking(Size, 10);
        this.dataTracker.startTracking(Particle, 0);
        this.dataTracker.startTracking(Gravity, false);
        this.dataTracker.startTracking(Glows, false);
        this.dataTracker.startTracking(Arrow, false);
        this.dataTracker.startTracking(Is3d, false);
        this.dataTracker.startTracking(Rotating, false);
        this.dataTracker.startTracking(Sticks, false);
    }
    ...
    public EntityProjectile(World level, LivingEntity limbSwingAmountEntityLiving, ItemStack item, boolean isNPC) {
        ...
        this.dataTracker.set(Arrow, this.getItem() == Items.ARROW);
    ...
    public void setThrownItem(ItemStack item) {
        this.dataTracker.set(ItemStackThrown, item);
    }
    ...
    public void tick() {
        ...
        if (this.inGround) {
            ...
            if (hitresult != null && hitresult.getType() != HitResult.Type.MISS) {
                this.dataTracker.set(Rotating, false);
            ...
            if (this.isTouchingWater()) {
                for (int j = 0; j < 4; ++j) {
                    float f4 = 0.25f;
                    this.getWorld().addParticle((ParticleEffect)ParticleTypes.BUBBLE, nextpos.x - motion.x * 0.25, nextpos.y - motion.y * 0.25, nextpos.z - motion.z * 0.25, motion.x, motion.y, motion.z);
                }
                f2 = 0.6f;
            }
    ...
    protected void onCollision(HitResult movingobjectposition) {
        block31: {
            block29: {
                block30: {
                    ...
                    if (this.isBlock()) {
                        ...
                        for (int i = 0; i < 8; ++i) {
                            this.getWorld().addParticle((ParticleEffect)new ItemStackParticleEffect(ParticleTypes.ITEM, this.getItemDisplay()), this.getX(), this.getY(), this.getZ(), this.random.nextGaussian() * 0.15, this.random.nextGaussian() * 0.2, this.random.nextGaussian() * 0.15);
                        }
                    }
            ...
            if (this.isArrow() || this.sticksToWalls()) {
                ...
                this.arrowShake = 7;
                if (!this.hasGravity()) {
                    this.dataTracker.set(Gravity, true);
                }
        ...
        if (this.explosiveRadius > 0) {
            ...
            this.getWorld().createExplosion((Entity)(this.getOwner() == null ? this : this.getOwner()), this.getX(), this.getY(), this.getZ(), (float)this.explosiveRadius, this.effect == 666, terraindamage ? World.ExplosionSourceType.TNT : World.ExplosionSourceType.NONE);
            if (this.effect != 0) {
                ...
                List<LivingEntity> list1 = this.getWorld().getNonSpectatingEntities(LivingEntity.class, axisalignedbb);
                StatusEffect p = PotionEffectType.getMCType(this.effect);
    ...
    public void readCustomDataFromNbt(NbtCompound compound) {
        ...
        if (compound.contains("inBlockState", 10)) {
            this.inBlock = NbtHelper.toBlockState((RegistryEntryLookup)this.getWorld().createCommandRegistryWrapper(RegistryKeys.BLOCK), (NbtCompound)compound.getCompound("inBlockState"));
        }
        this.throwableShake = compound.getByte("shake") & 0xFF;
        this.inGround = compound.getByte("inGround") == 1;
        this.dataTracker.set(Arrow, compound.getBoolean("isArrow"));
        ...
        this.dataTracker.set(Particle, compound.getInt("trailenum"));
        this.dataTracker.set(Size, compound.getInt("size"));
        this.dataTracker.set(Glows, compound.getBoolean("glows"));
        this.dataTracker.set(Velocity, compound.getInt("velocity"));
        this.dataTracker.set(Gravity, compound.getBoolean("gravity"));
        this.dataTracker.set(Is3d, compound.getBoolean("Render3D"));
        this.dataTracker.set(Rotating, compound.getBoolean("Spins"));
        this.dataTracker.set(Sticks, compound.getBoolean("Sticks"));
    ...
    public void getStatProperties(DataRanged stats) {
        ...
        this.dataTracker.set(Size, stats.getSize());
        this.dataTracker.set(Glows, stats.getGlows());
    ...
    public void setParticleEffect(int type) {
        this.dataTracker.set(Particle, type);
    }

    public void setHasGravity(boolean bo) {
        this.dataTracker.set(Gravity, bo);
    }

    public void setIs3D(boolean bo) {
        this.dataTracker.set(Is3d, bo);
    }

    public void setStickInWall(boolean bo) {
        this.dataTracker.set(Sticks, bo);
    }
    ...
    public void setSpeed(int speed) {
        this.dataTracker.set(Velocity, speed);
    }
    ...
    public void setRotating(boolean bo) {
        this.dataTracker.set(Rotating, bo);
    }
