...
implements RangedAttackMob {
    ...
    public EntityNPCInterface(EntityType<? extends PathAwareEntity> type, World world) {
        ...
        this.updateAI = true;
        this.bossInfo = new ServerBossBar(this.getDisplayName(), BossBar.Color.PURPLE, BossBar.Style.PROGRESS);
    ...
    private void registerBaseAttributes() {
        this.getAttributeInstance(EntityAttributes.GENERIC_MAX_HEALTH).setBaseValue((double)this.stats.maxHealth);
        this.getAttributeInstance(EntityAttributes.GENERIC_FOLLOW_RANGE).setBaseValue((double)CustomNpcs.NpcNavRange);
        this.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).setBaseValue((double)this.getMovementSpeed());
        this.getAttributeInstance(EntityAttributes.GENERIC_ATTACK_DAMAGE).setBaseValue((double)this.stats.melee.getStrength());
        this.getAttributeInstance(EntityAttributes.GENERIC_FLYING_SPEED).setBaseValue((double)(this.getMovementSpeed() * 2.0f));
    }

    public static DefaultAttributeContainer.Builder createMobAttributes() {
        return LivingEntity.createLivingAttributes().add(EntityAttributes.GENERIC_ATTACK_DAMAGE).add(EntityAttributes.GENERIC_FLYING_SPEED).add(EntityAttributes.GENERIC_FOLLOW_RANGE);
    }

    protected void initDataTracker() {
        super.initDataTracker();
        this.dataTracker.startTracking(RoleData, String.valueOf(""));
        this.dataTracker.startTracking(JobData, String.valueOf(""));
        this.dataTracker.startTracking(FactionData, 0);
        this.dataTracker.startTracking(Animation, 0);
        this.dataTracker.startTracking(Walking, false);
        this.dataTracker.startTracking(Interacting, false);
        this.dataTracker.startTracking(IsDead, false);
    ...
    public boolean tryAttack(Entity par1Entity) {
        ...
        RegistryEntry.Reference<DamageType> damageTypeHolder = this.getWorld().getRegistryManager().get(RegistryKeys.DAMAGE_TYPE).entryOf(NpcDamageSource.NPC);
        boolean var4 = par1Entity.damage(new DamageSource(damageTypeHolder, this), f);
        if (var4) {
        ...
        if (!this.getWorld().isClient) {
            if (!this.isKilled() && this.age % 20 == 0) {
                ...
                if (this.faction.getsAttacked && !this.isAttacking()) {
                    List<HostileEntity> list = this.getWorld().getNonSpectatingEntities(HostileEntity.class, this.getBoundingBox().expand(16.0, 16.0, 16.0));
            ...
            if (this.getHealth() <= 0.0f && !this.isKilled()) {
                ...
                this.dataTracker.set(IsDead, true);
                this.updateTasks();
                this.calculateDimensions();
            }
            ...
            this.dataTracker.set(Walking, !this.getNavigation().isIdle());
            this.dataTracker.set(Interacting, this.isInteracting());
    ...
    public boolean damage(DamageSource damagesource, float i) {
        ...
        try {
            ...
            if (i > 0.0f) {
                List<EntityNPCInterface> inRange = this.getWorld().getNonSpectatingEntities(EntityNPCInterface.class, this.getBoundingBox().expand(32.0, 16.0, 32.0));
    ...
    private void clearTasks(GoalSelector tasks) {
        ArrayList<PrioritizedGoal> list = new ArrayList<>(tasks.getGoals());
    ...
    private void updateTasks() {
        ...
        this.targetSelector.add(2, new NpcNearestAttackableTargetGoal<LivingEntity>(this, LivingEntity.class, 4, this.ais.directLOS, false, new NPCAttackSelector(this)));
    ...
    public void saySurrounding(Line line) {
        ...
        List<PlayerEntity> inRange = this.getWorld().getNonSpectatingEntities(PlayerEntity.class, this.getBoundingBox().expand(20.0, 20.0, 20.0));
    ...
    public void reset() {
        ...
        this.dataTracker.set(Animation, 0);
        this.dataTracker.set(Walking, false);
        this.dataTracker.set(IsDead, false);
        this.dataTracker.set(Interacting, false);
        this.interactingEntities.clear();
    ...
    private double calculateStartYPos(BlockPos pos) {
        ...
        while (pos.getY() > this.getWorld().getBottomY()) {
            ...
            if (this.ais.movementType == 2 && startPos.getY() <= pos.getY() && state.isOf(Blocks.WATER)) {
                pos = pos.down();
                continue;
            }
            return bb.maxY;
    ...
    public void setFaction(int id) {
        ...
        this.dataTracker.set(FactionData, id);
    }
    ...
    public boolean canHaveStatusEffect(StatusEffectInstance effect) {
        ...
        if (this.getGroup() == EntityGroup.ARTHROPOD && effect.getEffectType() == StatusEffects.POISON) {
            return false;
        }
    ...
    public void slowMovement(BlockState state, Vec3d motionMultiplierIn) {
        if (state != null && !state.isOf(Blocks.COBWEB) || !this.stats.ignoreCobweb) {
    ...
    public PistonBehavior getPistonBehavior() {
        return this.display.getHitboxState() == 0 ? super.getPistonBehavior() : PistonBehavior.IGNORE;
    }
    ...
    public void setRoleData(String s) {
        this.dataTracker.set(RoleData, s);
    }

    public String getJobData() {
        return (String)this.dataTracker.get(JobData);
    }

    public void setJobData(String s) {
        this.dataTracker.set(JobData, s);
    }
    ...
    public void setDataWatcher(DataTracker entityData) {
        ArrayList<DataTracker.SerializedEntry<?>> list = new ArrayList<>();
    ...
    protected void updateGoalControls() {
        ...
        this.goalSelector.setControlEnabled(Goal.Control.JUMP, flag1);
